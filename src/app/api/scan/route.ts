...
export async function POST(request: Request) {
  try {
    ...
    // ── Phase 1: Security Header Check (EXISTING — UNCHANGED) ─────────────
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    ...
    try {
      ...
      try {
        ...
        return NextResponse.json({ error: 'Failed to reach the website. Please check the URL and try again.' }, { status: 500 });
      }
    }

    if (!response) {
      return NextResponse.json({ error: 'No response received from the website.' }, { status: 500 });
    }

    // Define the critical headers we are looking for (UNCHANGED)
    ...
    const trackerPenalty = Math.min(trackerResult.found * 6.25, 25); // 4 trackers = 0 points
    const trackerComponent = 25 - trackerPenalty;

    const totalScore = Math.round(headerComponent + gpcComponent + trackerComponent);

    // Calculate Grade based on custom percentage ranges (SAME THRESHOLDS)
    let grade = 'F';
